...
class LevlWorkflowStateConverterTest {
    ...
    public void asProperties() {
        ...
        assertThat(properties.stream().map(TestObjects::describe)).containsExactly(
                "primary.use.case.active.power.w=222",
                "next.discharge.power.w=333",
                "actual.levl.power.w=111",
                ...
                "current.request.realized.discharge.energy.ws=\"4\"",
                "last.request.realized.discharge.energy.ws=\"5\"",
                "last.discharge.request.timestamp=\"2024-02-15T12:00:00Z\"",
                "efficiency.percent.multiplied.by.hundred=9903",
                "current.discharge.request.id=\"id0\"",
                "current.discharge.request.timestamp=\"2024-02-15T15:00:00Z\"",
                "current.discharge.request.energy.ws=\"9\"",
                "current.discharge.request.start=\"2021-01-01T00:00:10\"",
                "current.discharge.request.deadline=\"2021-01-01T00:00:11\"",
                "current.discharge.request.active=true",
                "next.discharge.request.id=\"id1\"",
                "next.discharge.request.timestamp=\"2024-03-13T18:00:00Z\"",
                "next.discharge.request.energy.ws=\"6\"",
                "next.discharge.request.start=\"2021-01-01T00:00:07\"",
                "next.discharge.request.deadline=\"2021-01-01T00:00:08\"",
                "next.discharge.request.active=false",
                "levl.soc.constraints.lower.physical.percent=1",
                "levl.soc.constraints.upper.physical.percent=100",
                "levl.soc.constraints.lower.logical.percent=5",
                "levl.soc.constraints.upper.logical.percent=95",
                "grid.power.limit.w.lower=-1100",
                "grid.power.limit.w.upper=1200"
        );
    }
    ...
    public void fromProperties() {
        ...
        assertThat(state).hasToString("LevlWorkflowStateMemento[primaryUseCaseActivePowerW=222, nextDischargePowerW=333, actualLevlPowerW=111, " +
    ...
    public void fromProperties_CurrentRequest_StartTimestampInvalid() {
        var config = StorageConfigTestBuilder.aDefaultStorageConfig().withCurrentDischargeRequestStart("no date").build();
        var converter = new LevlWorkflowStateConverter();
